...
class MySet(data.Dataset):
    ...
    def __init__(self, is_train, data_dir, image_size, color_jitter_brightness, color_jitter_saturation, laplacia_level_count):
        self.img_pths = [os.path.join(data_dir, img_name) for img_name in os.listdir(data_dir)]
        image_size = (image_size, image_size) if isinstance(image_size, int) else image_size
        self.color_jitter = T.ColorJitter(brightness=color_jitter_brightness, saturation=color_jitter_saturation)
...
def make_loader(is_train, data_dir, image_size, color_jitter_brightness, color_jitter_saturation, batch_size, laplacia_level_count, num_workers):
    loader = iter(data.DataLoader(MySet(is_train, data_dir, image_size, color_jitter_brightness, color_jitter_saturation, laplacia_level_count), batch_size=batch_size, shuffle=True, drop_last=True, num_workers=num_workers))
    return loader
...
if __name__ == "__main__":
    ...
    loader = make_loader(True, data_dir, 512, 0.85, 0.85, 8, laplacian_level_count, num_workers=4)
    to_pil = T.ToPILImage()
